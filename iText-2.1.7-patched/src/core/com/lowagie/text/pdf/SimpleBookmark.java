 ...
 * code and are released according to the original license of itext-2.1.7.
 * 
    ...
    static PdfDictionary createActionDictionary(HashMap<String, Object> map, PdfWriter writer, boolean namedAsNames) {
        ...
        try {
            ...
            if ("GoTo".equals(action)) {
                ...
                if (fieldName != null) {
                    dic.put(PdfName.S, PdfName.HIDE);
                    dic.put(PdfName.T, new PdfString(fieldName));
                    String hide = (String) map.get("Hide");
                    if (hide.equals("false")) {
                        dic.put(PdfName.H, PdfBoolean.PDFFALSE);
                    }
                }
            }
